...
document.addEventListener("DOMContentLoaded", () => {
  function initializeServicesContainer(container) {
    ...
    function updateVisibility() {
      const isMobile = window.matchMedia("(max-width: 768px)").matches; // Check for mobile screens
      serviceCards.forEach((card, index) => {
        ...
        ["left", "right"].forEach((direction) => {
          ...
          // Show/hide arrows based on conditions
          if (
            extraContentOpen ||
            ...
            arrow.style.display = "none"; // Hide on extra content or larger screens
          } else {
            arrow.style.display = "block"; // Show on smaller screens and when not at start/end
          }
        });
      });
    }
...
//           // Hide the arrows if extra content is open or at the first/last card
//           if (
//             extraContentOpen ||
//             (direction === "left" && currentIndex === 0) ||
//             (direction === "right" && currentIndex === totalCards - 1)
